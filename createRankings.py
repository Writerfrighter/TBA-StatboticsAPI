...
def normalizeData(data):
    ...
    data_normalized = {
        team_number[3::]: data[team_number] - mean_data for team_number in data
    }
...
def fetchTeam_Threaded(
    ...
    if useEndgame_EPA:
        endgame_epas_max[team] = float(team_query["endgame_epa_max"])
    logging.info("Thread %s: finished", name)


def createRankings(
    event, useOPR, useCCWMS, useOverall_EPA, useAuto_EPA, useTeleOp_EPA, useEndgame_EPA
):
    ...
    if TBA.checkAPIStatus() != True:
        if (
            not useOPR
            and not useCCWMS
            and not useOverall_EPA
            and not useAuto_EPA
            and not useTeleOp_EPA
            and not useEndgame_EPA
        ):
        ...
        if useOPR:
            ...
            oprs_normalized = dict.fromkeys(np.arange(0, team_count + 1), 0)
        if useCCWMS:
            ccwms = API_Response["ccwms"]
            ccwms_normalized = normalizeData(ccwms)
        else:
            ccwms_normalized = dict.fromkeys(np.arange(0, team_count + 1), 0)

        global team_names, epas_max, auto_epas_max, teleop_epas_max, endgame_epas_max
        team_names = {}

        if not useOverall_EPA:
            epas_max = dict.fromkeys(np.arange(0, team_count + 1), 0)
        else:
            epas_max = {}
        if not useAuto_EPA:
            auto_epas_max = dict.fromkeys(np.arange(0, team_count + 1), 0)
        else:
            auto_epas_max = {}
        if not useTeleOp_EPA:
            teleop_epas_max = dict.fromkeys(np.arange(0, team_count + 1), 0)
        else:
            teleop_epas_max = {}
        if not useEndgame_EPA:
            endgame_epas_max = dict.fromkeys(np.arange(0, team_count + 1), 0)
        else:
            endgame_epas_max = {}
        ...
        if useOverall_EPA:
            tofetch.append("epa_max")
        if useAuto_EPA:
            tofetch.append("auto_epa_max")
        if useTeleOp_EPA:
            tofetch.append("teleop_epa_max")
        if useEndgame_EPA:
            tofetch.append("endgame_epa_max")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=team_count) as executer:
            executer.map(
                ...
                [event] * team_count,
                [tofetch] * team_count,
                [useOverall_EPA] * team_count,
                [useAuto_EPA] * team_count,
                [useTeleOp_EPA] * team_count,
                [useEndgame_EPA] * team_count,
            )

        logging.info("Main: finished Statbotics API fetch")

        team_names = {
            key: val for key, val in sorted(team_names.items(), key=lambda ele: ele[0])
        }

        if useOverall_EPA:
            epa_max_normalized = normalizeData(epas_max)
        else:
            epa_max_normalized = epas_max
        if useAuto_EPA:
            auto_epa_max_normalized = normalizeData(auto_epas_max)
        else:
            auto_epa_max_normalized = auto_epas_max
        if useTeleOp_EPA:
            teleop_epa_max_normalized = normalizeData(teleop_epas_max)
        else:
            teleop_epa_max_normalized = teleop_epas_max
        if useEndgame_EPA:
            endgame_epa_max_normalized = normalizeData(endgame_epas_max)
        else:
            endgame_epa_max_normalized = endgame_epas_max

        names = [
            "{} ({})".format(team_names[team_number], team_number[3::])
            for team_number in team_names.keys()
        ]
        ...
        scores = [
            ...
            for (
                ...
                oprs_normalized.values(),
                ccwms_normalized.values(),
                epa_max_normalized.values(),
                auto_epa_max_normalized.values(),
                teleop_epa_max_normalized.values(),
                endgame_epa_max_normalized.values(),
            )
        ]
